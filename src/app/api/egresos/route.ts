...
export async function GET(req: NextApiRequest) {
    try {
        ...
        const [registroCompras]: any[] = await connection.query(`
    SELECT f.id, f.fecha, f.monto, mp.nombre AS metodo_de_pago, f.descripcion
    FROM finanzas f
    LEFT JOIN registro_de_compras rc ON f.compra_id = rc.id
    JOIN metodo_pago mp ON mp.id = rc.metodo_pago_id
    WHERE f.monto < 0;

`)
        await connection.end();
        return NextResponse.json(registroCompras);
    } catch (error) {
...
export async function POST(req: Request, res: NextApiResponse) {
    ...
    try {
        ...
        const [resultVenta]: any = await connection.query(
            `
        INSERT INTO registro_de_compras (fecha, importe, metodo_pago_id) 
        VALUES (?, ?, ?)
        `,
